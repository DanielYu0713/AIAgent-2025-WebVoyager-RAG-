...
from langchain.text_splitter import TokenTextSplitter  # ← HW3
...
# 將 PDF 檔案轉換成各種可操作的中間格式（Markdown、純文字）並擷取其中的圖片路徑
class DocumentConverter:
    ...
    def pdf_to_markdown( #PDF 轉 Markdown
            self,
            pdf_path: str,
    ...
    def pdf_to_text(self, pdf_path: str, use_ocr: bool = True, ocr_lang: str = "chi_tra+jpn") -> List[Tuple[int, str]]:
...
class EmbeddingFactory:
    ...
    def create(
        ...
        Args:
            embedding_type (Literal["openai"]): Embedding type ("openai")
...
class TextSplitter:
    ...
    def __init__(self,
                 ...
                 splitter_type: Literal["token", "recursive"] = "token"): #HW3 splitter_type 
        """
        ...
        """
        Args:
            splitter_type: "token"  → TokenTextSplitter (預設)
        ...
        if splitter_type == "recursive":
            ...
            self.splitter = TokenTextSplitter(
                ...
                chunk_overlap=chunk_overlap,
                length_function=len,
            )#RecursiveCharacterTextSplitter -> TokenTextSplitter；chunk_size = 1000 => 500, chunk_overlap=200 => 100
        #HW3
    ...
    def split_text(self, text: str, metadata: Dict[str, Any] = None) -> List[Document]:
        ...
        if metadata is None:
            metadata = {}
            # 新增檔案最後修改時間，HW3
            ts = os.path.getmtime(document_path)
            metadata["timestamp"] = datetime.fromtimestamp(ts).isoformat()
...
class RAGEngine:
    ...
    def _index_pdf(
        ...
        """
        # HW3 新增：把 PDF 最後修改時間加到 metadata
    ...
    def _index_markdown(
        ...
        Args:
            ...
            metadata (Dict[str, Any]): Metadata
        """
         # HW3 新增：把 Markdown 檔案最後修改時間加到 metadata
        metadata["timestamp"] = datetime.fromtimestamp(
    ...
    def search(
        ...
        current_intent: str = "",  # ★ 新增：當前代理意圖
        summarize: bool = False,   # ★ 新增：是否要做「摘要」
        return_raw: bool = False,  # ★ 新增：是否回傳原始 Document 物件
    ) -> List[Dict[str, Any]]:
        """
        在向量資料庫中，以語意搜尋取得最相關的 chunks，
        並依 section 命中情況 rerank，再對長 chunk 做簡易摘要。
        
        Args:
            query: 要搜尋的文字
            k: 最終要回傳的筆數
            filter_dict: 過濾 metadata 的 dict（可為 None）
            summarize: 若 True，對長度 >400 的 chunk 做截取
            return_raw: 若 True，額外回傳原始 Document 物件
        
        Returns:
            List of {
                "content": str,      # chunk 文字（可能經 summarize）
                "metadata": dict,    # chunk metadata
                "source": str,       # 檔案路徑
                "section": str,      # 章節名稱
                "page": int or str,  # 頁碼
                "_raw_document": Document  # 若 return_raw=True
            }
        """
        # 1. 先用 k*2 拿更多結果
        search_kwargs = {}
        ...
        # 2. Rerank：如果 query 在 metadata["section"]，加 0.1 分
        # def _rerank(doc):
        ...
        reranked = sorted(raw_results, key=_rerank)[:k]
        ...
        for doc in reranked:
            ...
            # 4. 如果需要摘要，且 chunk 太長，就做前後各 200 字截取
            ...
            item = {
                ...
                "intent_matched": current_intent in chunk,  # ★ 標記這筆是否匹配意圖 
            }

            # 5. 若要回傳原始 Document 物件（除錯用）
            if return_raw:
                item["_raw_document"] = doc
...
class PDFEnhancementPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            embedding_type (Literal["openai"], optional):
    ...
    def search(
        ...
        """
        Wraps RAGEngine.search，並把所有可選參數往下游傳遞
        """
